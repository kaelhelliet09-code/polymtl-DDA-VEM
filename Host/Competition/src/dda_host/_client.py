...
class CompetitionBoard:
    ...
    Technician programs normally select ``DeviceMode.DEBUG`` so coil and sensor
    commands remain available at all times. In ``DeviceMode.COMPETITION``, those
    commands are accepted only while a launch is active.
    """
    ...
    def drive(
        ...
        if selected_bridge is Bridge.ALL and self._inverted_bridge_mask not in (
            0,
            0x0F,
        ):
            bridges = (Bridge.H1, Bridge.H2, Bridge.H3, Bridge.H4)

        for current_bridge in bridges:
            inverted = (
                self._inverted_bridge_mask == 0x0F
                if current_bridge is Bridge.ALL
                else bool(self._inverted_bridge_mask & (1 << int(current_bridge)))
            )
            forward = (selected_direction is Direction.FORWARD) != inverted
            command = _CoilCommand.FORWARD if forward else _CoilCommand.REVERSE
            self._expect_success(
                Service.COILS, command, int(current_bridge), "drive bridge"
            )

    def invertDirection(self, bridge: Bridge | int) -> None:
        """Invert future direction commands for one bridge or all bridges."""

        selected_bridge = Bridge(bridge)
        mask = 0x0F if selected_bridge is Bridge.ALL else 1 << int(selected_bridge)
        self._inverted_bridge_mask ^= mask
    ...
    def inverserDirection(self, bobine: Bobine | int) -> None:
        """Inverse le sens des prochaines commandes de la bobine."""

        self.invertDirection(bobine)
