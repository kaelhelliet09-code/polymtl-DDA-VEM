...
class CompetitionBoardTests(unittest.TestCase):
    ...
    def test_direction_inversion_is_local_and_toggleable(self) -> None:
        connection = FakeCompetitionSerial()
        board = CompetitionBoard(connection)
        try:
            board.inverserDirection(Bobine.H3)
            self.assertEqual(connection.writes, [])

            board.activer(Bobine.H3, Direction.AVANT)
            board.activer(Bobine.H3, Direction.ARRIERE)
            board.inverserDirection(Bobine.H3)
            board.activer(Bobine.H3, Direction.AVANT)

            self.assertEqual(connection.writes[0], bytes((1, 1, 0x82)))
            self.assertEqual(connection.writes[1], bytes((1, 0, 0x82)))
            self.assertEqual(connection.writes[2], bytes((1, 0, 0x82)))
        finally:
            board.close()

    def test_direction_inversion_handles_all_bridges(self) -> None:
        connection = FakeCompetitionSerial()
        board = CompetitionBoard(connection)
        try:
            board.inverserDirection(Bobine.H2)
            board.activer(Bobine.TOUTES, Direction.AVANT)
            ...
            board.inverserDirection(Bobine.H2)
            board.invertDirection(Bridge.ALL)
            board.forward(Bridge.ALL)
            self.assertEqual(connection.writes, [bytes((1, 1, 0x84))])
        finally:
            board.close()
