// Implements exclusive ADC DMA ownership and validates the revised ADC map.
...
constexpr uint32_t ExpectedBridgeCurrentChannels =
    ADC_CHSELR_CHSEL0 | ADC_CHSELR_CHSEL4 | ADC_CHSELR_CHSEL11 |
    ADC_CHSELR_CHSEL18;
...
AdcSampler::AdcSampler(ADC_HandleTypeDef &handle) noexcept
    : _handle(handle), _sequenceEvent(SequenceEvent::None), _buffer(nullptr),
      _valueCount(0U), _completedValueCount(0U), _activeSegmentLength(0U) {}
...
HAL_StatusTypeDef AdcSampler::validateConfiguration() const noexcept {
  if ((_handle.DMA_Handle == nullptr) || (_handle.Instance == nullptr)) {
    return HAL_ERROR;
  }
  DMA_HandleTypeDef &dma = *_handle.DMA_Handle;
  const uint32_t selectedChannels =
      _handle.Instance->CHSELR & ADC_CHSELR_CHSEL;
  const bool valid =
      (dma.Instance == DMA1_Channel2) &&
      (_handle.Init.Resolution == ADC_RESOLUTION_8B) &&
      (_handle.Init.DataAlign == ADC_DATAALIGN_RIGHT) &&
      (_handle.Init.ScanConvMode == ADC_SCAN_SEQ_FIXED) &&
      (_handle.Init.EOCSelection == ADC_EOC_SEQ_CONV) &&
      (_handle.Init.ContinuousConvMode == DISABLE) &&
      (_handle.Init.ExternalTrigConv == ADC_EXTERNALTRIG_T15_TRGO) &&
      (_handle.Init.ExternalTrigConvEdge == ADC_EXTERNALTRIGCONVEDGE_RISING) &&
      (_handle.Init.DMAContinuousRequests == ENABLE) &&
      (selectedChannels == ExpectedBridgeCurrentChannels) &&
...
uint32_t AdcSampler::transferredValueCount() const noexcept {
  ...
  return transferred < _valueCount ? transferred : _valueCount;
}
...
HAL_StatusTypeDef AdcSampler::stop() noexcept {
  ...
  return ownedByAnotherWrapper ? HAL_BUSY : HAL_OK;
}
